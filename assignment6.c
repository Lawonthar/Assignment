...
void registration();
void lobby();
void emailVerification(char toCheckEmail[50]);
...
void emailVerification(char toCheckEmail[50]){
    ...
    char gmail[9]={'g','m','a','i','l','.','c','o','m'};

   int emailNum= charCounting(toCheckEmail);

   int sameNum=0;
   int j=8;
   for(int i=emailNum-1 ; i>emailNum-10; i--){


       if(toCheckEmail[i]==gmail[j]){
...
void registration(){
   ...
   myStrCmp(tempEmail);
    ...
    if(eFound == -1){
        printf("U can register\n");

        for(int gcc=0 ; gcc<50 ;gcc++){
            info[globalIndex].email[gcc]=tempEmail[gcc];
        }
        ...
        printf("email already taken!\nRetry!");
        registration();
    }

}

void myStrCmp(char *userInputChar) {
